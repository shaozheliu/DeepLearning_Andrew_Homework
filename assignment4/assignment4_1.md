...
```python
# GRADED FUNCTION: initialize_parameters  初始化两层网络的参数
...
def initialize_parameters(n_x, n_h, n_y):
    ...
    b1 = np.zeros((n_h,1))   # 传入的参数要是元组，否则会报错
...
```python
# GRADED FUNCTION: initialize_parameters_deep  网络总共有L层，
...
def L_model_forward(X, parameters):
    ...
    ### START CODE HERE ### (≈ 2 lines of code)
    AL, cache = linear_activation_forward(A,parameters['W'+str(L)],parameters['b'+str(L)],'sigmoid')  # 注意这里是最后一层，str(L)
    caches.append(cache)
